...
Under many circumstances, not much. In the OLS estimator is identical to the ML estimator under the normality assumption for the error terms. To see this, recall we've been considering the model

$$ y = x \beta + \epsilon $$

Assuming $\epsilon \sim N(0,\sigma^{2})$, the $$\log(\mathcal{L})$$ of $y|x$ can be reduced to
...
In practice, a good grasp on ML methods may help you get a better sense of what you're asking R or Stata to do trying to fit quantile regressions, negative binomial regressions or Heckman selection models. This will surely come in handy ~~when~~ if you're pulling your hair out while trying to figure out why your estimations won't converge!
