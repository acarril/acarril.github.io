...
we can think of it as a convenient way of modeling $$E[y\mid x]$$, or a way of predicting $$y$$ based on $$x$$'s values. However, in applied econometrics it is usally meant to answer a question of causality: "by how much does a change in $$x$$ affect $$y$$?"
...
A perfect (?) **randomized controlled trial** is a way of *forcing* 2 and 3 to be true: if you pick $$x$$ randomly, then it surely isn't being caused by $$y$$, $$\epsilon$$... or any other thing, for that matter. So-called **natural experiments** are cases when special circumstances arise in which 2 and 3 seem plausible, even though that usually isn't the case.
