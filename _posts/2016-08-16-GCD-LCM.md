...
However, it is more interesting to compute these results for multiple inputs. Programs `gcdm` and `lcmm` do so, exploiting the fact that both operations are associative:

$$\gcd(a,b,c) = \gcd(a,\gcd(b,c))$$

$$\lcm(a,b,c) = \lcm(a,\gcd(b,c))$$
