...
To install, just bring up the Command Palette with `Ctrl+Shift+P`, type "install", hit `Enter` and search for "Anaconda". Just hit `Enter` again to install, and restart ST to see the changes.
