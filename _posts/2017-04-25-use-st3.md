...
Getting [Stata](http://www.stata.com/) to work with ST3 requires a few steps, but it works very well when all is said and done. The process is quiet different for Windows and OSX, so I've split this tutorial in two sections. I'm assuming that you already have Stata up and running (this works for all versions).
