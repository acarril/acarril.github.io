...
Getting [Stata](http://www.stata.com/) to work with ST3 requires a few steps, but it works very well when all is said and done. The process is quite different for [Windows](#windows) and [OSX](#os-x), so I've split this tutorial in two sections. I'm assuming that you already have Stata up and running (these steps will work with all versions).
...
In OS X there are many ST3 plugins for Stata, but I like the relatively new [Improved Stata Editor](https://packagecontrol.io/packages/Stata%20Improved%20Editor). It works very well and has a ton of useful features, so be sure to check its page for a full list of features.

Installation is extremely easy: just open the command palette via Tools > Command Palette (or with `Cmd+Shift+P`) and type "install" to search for the "Package Control: Install Package" option. Hit enter and search for "Stata Improved Editor", selecting it and clicking Enter to install. Reboot ST3 and you're ready!

Open any existing do-file or create a new file with `.do` extension to test it out. You should see "Stata" in the bottom right corner, indicating ST3 detected the syntax for the `.do` format. You can run whole scripts or just chunks of code with `Ctrl+d`. Again, be sure to check out the [Improved Stata Editor's homepage](https://github.com/zizhongyan/StataImproved) to see many other features.
